...
"""
class Airline:
    def __init__(self, flights=0, on_times=0, name=None):
        self.flights = flights
        self.on_times = on_times
        self.name = name

    def get_percentage(self):
        return (self.on_times/self.flights)*100
...
class ThreadingSolution:
    ...
    def __init__(self, num_of_threads=None, dataset_path=None, dataset_size=None):
        ...
        self.dataset_columns = ["FlightDate", "Airline", "ArrDelayMinutes"]  # only import the columns that are necessary 
    
    """
    Returns the tuple of computed result and time taken. eg., ("I am final Result", 3.455)
    """
    def run(self):
        print("Iymen Abdella | 40218280 | COMP 6231 FALL 2023 | Assignment 2: Parallel Programming | Q2 T1")
        threads = []
        lock = threading.Lock()
        ...
        for airline in filtered_df['Airline'].unique():
            ...
            self.airlines.append(Airline(flights=flight_count, on_times=0, name=airline))
    ...
    def filter_csv(self, df):
        filter = ((pd.to_datetime(df["FlightDate"]).dt.year == 2021) & df['ArrDelayMinutes'] == 0)  
        return df[filter]
    ...
    def count_flights(self, df, start_index, end_index, lock, thread_index):
        ...
        for i in range(start_index, end_index): # 0 index is header
            row = df.iloc[i, :]
            lock.acquire()
            [airline for airline in self.airlines if airline.name == row["Airline"]][0].on_times += 1 
            lock.release()
