...
class MultiProcessingSolution:
    ...
    """
    def run(self):
        print("Iymen Abdella | 40218280 | COMP 6231 FALL 2023 | Assignment 2: Parallel Programming | Q3 T2")
    ...
    def count_flights(self, chunk):
        ...
        for airline in chunk['Airline'].unique():
            ...
            dep_count = ((pd.to_datetime(chunk["FlightDate"]) >= start_date) & (pd.to_datetime(chunk["FlightDate"]) <= end_date) & (chunk['ArrDelayMinutes'] < 0)).sum()
