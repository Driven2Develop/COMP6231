...
def handle_dl(current_working_directory, file_name, service_socket, eof_token):
    ...
    :param eof_token: a token to indicate the end of the message.
    """
def handle_info(current_working_directory, file_name, service_socket, eof_token):
    
    file_path = os.path.join(current_working_directory, file_name)
    if (os.path.isfile(file_path)):
        send_message_ending_with_token(service_socket, str(os.path.getsize(file_path)), eof_token)
        print("Successfully sent file size info to client")
    else:
        print("Invalid Entry. File does not exist.")
        send_message_ending_with_token(service_socket, str(-1), eof_token)
    ...
    """
def handle_mv(current_working_directory, full_command, service_socket, eof_token):
    
    from_file = full_command.split(" ")[0]
    to_file = full_command.split(" ")[1]
    to_file_path = os.path.join(current_working_directory, to_file)
    from_file_path = os.path.join(current_working_directory, from_file)

    #check if file from and destination exist. 
    if not os.path.isfile(from_file_path):
        print("Invalid entry. File {} or {} do(es) not exist".format(from_file, to_file))
        return

    if os.path.isdir(to_file_path):
        os.rename(from_file_path, os.path.join(os.path.dirname(from_file_path), os.path.join(to_file, from_file)))
        print('Successfully moved file to directory {}'.format(to_file))
    else:
        os.rename(from_file_path, to_file_path)
        print("Successfully renamed file to {}".format(to_file))
    ...
    def run(self):
        ...
        while True:
            ...
            if cmd == "cd":
                ...
                handle_info(self.cwd, client_message[len('info'):].strip().lower(), self.service_socket, self.eof_token)
            elif cmd == "mv":
                handle_mv(self.cwd, client_message[len('mv'):].strip().lower(), self.service_socket, self.eof_token)
            elif cmd == "exit":
                break
