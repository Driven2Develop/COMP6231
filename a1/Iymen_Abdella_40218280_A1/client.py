...
def issue_info(command_and_arg, client_socket, eof_token):
    send_message_ending_with_token(client_socket, command_and_arg, eof_token)
    message = receive_message_ending_with_token(client_socket, 1024, eof_token)
    if (message.startswith('-1')):
        print("Invalid entry. File not available.\n")
    else:
        print("File has size {} bytes.\n".format(message))

    
def issue_mv(command_and_arg, client_socket, eof_token):
    send_message_ending_with_token(client_socket, command_and_arg, eof_token)
    print(receive_message_ending_with_token(client_socket, 1024, eof_token))
