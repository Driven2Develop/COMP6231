...
class Server:
    ...
    def start(self):
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as self.server_socket:
            ...
            print("Server binded to host {} on port {}. Listening for connections...".format(self.host, self.port))

            while True:
                conn, addr = self.server_socket.accept() # client is connected, so create new thread

                print("Connected by address: {}. Creating new client thread.".format(addr))
                thread = ClientThread(self, self.server_socket, addr, self.generate_random_eof_token())
                thread.run()
    ...
    """
    def generate_random_eof_token(self, eof_token_length=10):
    ...
    def handle_ul(
        ...
        with open(os.path.join(current_working_directory, file_name), 'w') as file:
            content = self.receive_message_ending_with_token(service_socket, 1024, eof_token).decode()
            file.write(content)
...
class ClientThread(Thread):
    ...
    def run(self):
        ...
        self.server_obj.receive_message_ending_with_token
        self.server_obj.send_message_ending_with_token(self.service_socket, self.server_obj.get_working_directory_info(self.cwd), self.eof_token)
        
        #main execution loop
        while True:
            client_message = self.server_obj.receive_message_ending_with_token(self.service_socket, 1024, self.eof_token).decode()
            
            # get the command and arguments and call the corresponding method
            cmd = client_message.split(" ")[0].strip().lower()

            if cmd == "cd":
                self.cwd = self.server_obj.handle_cd(self.cwd, client_message[len('cd'):].strip().lower() )
            elif cmd == "mkdir":
                self.server_obj.handle_mkdir(self.cwd, client_message[len('mkdir'):].strip().lower())
            elif cmd == "rm":
                self.server_obj.handle_rm(self.cwd, client_message[len('rm'):].strip().lower())
            elif cmd == "ul":
                self.server_obj.handle_ul(self.cwd, client_message[len('ul'):].strip().lower(), self.service_socket, self.eof_token)
            elif cmd == "dl":
                self.server_obj.handle_dl(self.cwd, client_message[len('dl'):].strip().lower(), self.service_socket, self.eof_token)
            elif cmd == "info":
                self.server_obj.handle_info(self.cwd, client_message[len('info'):].strip().lower(), self.service_socket, self.eof_token)
            elif cmd == "mv":
                self.server_obj.handle_mv(self.cwd, client_message[len('mv'):].strip().lower(), self.service_socket, self.eof_token)
            elif cmd == "exit":
                break
            
            # after every operation, send current dir info
            self.server_obj.send_message_ending_with_token(self.service_socket, self.get_working_directory_info(self.cwd), self.eof_token)
